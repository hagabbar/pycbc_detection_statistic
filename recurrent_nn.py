#Simple Multilayer Recurrent Neural Network to separate pycbc single detector injections from single detector noise triggers
...
import numpy as np
import h5py
from keras.layers import Dense, Activation, Dropout, GaussianDropout, ActivityRegularization, LSTM
...
def the_machine(args, n_features, train_weights, test_weights, train_data, test_data, lab_train, lab_test, out_dir, now):
    ...
    print n_features
    input_len = train_data.shape[0]

    model.add(LSTM(7, input_dim=train_data.shape, return_sequences=True, input_length=input_len)) #Add in Dense 7
    #act
...
def main():
    #For TESTING ONLY
    shutil.rmtree('/home/hunter.gabbard/public_html/simple_neural_net/testing/classification/L1-all_of_O1_run/recurrent_nn/run_test1')
